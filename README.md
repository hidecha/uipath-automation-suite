...
│   ├── main.tf                   # Provider 定義
│   ├── variables.tf              # 変数定義
│   ├── network.tf                # VPC / Subnet / Security Group / Pod CIDR
...
│
├── docs/                         # デプロイガイド
│   ├── EKS_DEPLOY.md             # EKS デプロイ手順書
│   ├── AKS_DEPLOY.md             # AKS デプロイ手順書
│   └── EKS_ERROR_DEPLOY.md       # EKS デプロイ時のエラー対処
│
├── .gitignore
└── README.md
```
...
|---|---|---|
| ネットワーク | VPC, Public/Private Subnet, Pod CIDR Subnet, IGW, NAT GW | VNet, Subnet |
...
| `pod_cidr` | Pod IP 用セカンダリ CIDR | `100.64.0.0/16` |
...
| `db_instance_type` | SQL Server インスタンスタイプ | `db.m6i.2xlarge` |
| `redis_password` | Redis 認証トークン (16文字以上) | - |
| `eks_fqdn` | Automation Suite の FQDN | `as.example.com` |
| `kubernetes_version` | EKS バージョン | `1.34` |
| `cpu_instance_type` | CPU ノードの EC2 インスタンスタイプ | `c7a.8xlarge` |
| `number_of_cpu_nodes` | CPU ノード数 | `3` |
| `gpu_instance_type` | GPU ノードの EC2 インスタンスタイプ | `g4dn.xlarge` |
| `number_of_gpu_nodes` | GPU ノード数 (不要なら `0`) | `0` |
| `asrobot_instance_type` | AS Robot ノードの EC2 インスタンスタイプ | `c7a.4xlarge` |
| `number_of_asrobot_nodes` | AS Robot ノード数 (不要なら `0`) | `0` |
| `postgres_instance_type` | PostgreSQL インスタンスタイプ | `db.m6i.xlarge` |
| `postgres_engine_version` | PostgreSQL エンジンバージョン | `16.8` |
| `postgres_username` / `postgres_password` | PostgreSQL 管理者認証情報 | - |
| `postgres_storage_size` | PostgreSQL ストレージサイズ (GB) | `100` |
| `postgres_max_storage` | PostgreSQL 最大ストレージサイズ (GB) | `500` |
...
| `pod_subnets` | AZ ごとの Pod サブネット ID (ENIConfig 用) |
...
| `kubernetes_version` | AKS Kubernetes バージョン | `1.34` |
| `number_of_cpu_nodes` | CPU ノード数 | `3` |
| `number_of_asrobot_nodes` | AS Robot ノード数 (不要なら `0`) | `0` |
